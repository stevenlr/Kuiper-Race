...
namespace {
	...
	char *loadPng(const std::string &filename, Texture::InternalFormat &internalFormat, Texture::Format &format, int &width, int &height)
	{
...
Texture *loadPngTexture(const std::string &filename, bool mipmaps)
{
	...
	char *data = loadPng(filename, internalFormat, format, width, height);
...
Cubemap *loadPngCubemap(std::initializer_list<std::string> filenames, bool mipmaps)
{
	...
	for (int i = 0; i < 6; ++i) {
		char *data = loadPng(*filenamesIterator++, internalFormat, format, width, height);
