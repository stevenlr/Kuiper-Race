...
void Segment::overwriteAsteroid(unsigned short asteroidIndex)
{
	asteroids[asteroidIndex]=asteroids.back();
	asteroids.pop_back();
}

void Segment::removeTurnCollisions(Segment & previousSegment)
{
	for(unsigned short i=0; i<previousSegment.asteroids.size(); ++i)
	{
		for(unsigned short j=0; j<asteroids.size(); ++j)
		{
			if(detectOneAsteroidCollision(previousSegment.asteroids[i],
										  asteroids[j]))
			{
				overwriteAsteroid(j);
				--j;
